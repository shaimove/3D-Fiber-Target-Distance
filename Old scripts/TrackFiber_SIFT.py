...
list_img = np.array([20,21,40,48,49,56,64,66,74,81,82,89,147,163,184,189
                     ,205,241,250,252,258,321,323,330,343,366,374,377,379,
                     391,393,394,397,398,414,429,443])

BOW_NUM_TRAINING_SAMPLES_PER_CLASS = len(list_img)

#%% helpping dunctions
def get_pos_and_neg_paths(i):
    num = list_img[i]
    pos_path = 'Positive/pos-%d.jpg' % (num)
    neg_path = 'Negative/neg-%d.jpg' % (num)
...
training_data = []
training_labels = []
SVM_NUM_TRAINING_SAMPLES_PER_CLASS = len(list_img)
...
for i,test_img_path in tqdm(enumerate(list_of_images)):
    
    img = test_img_path
    gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    pos_rects = []
    min_size=(40, 40); max_size=(100, 100)
